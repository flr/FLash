...
An important part of stock assessment is conducting stock projections to advise on management regulations e.g. catches, effort or changes to selection patterns.
...
```
For the examples we use the ple4 FLSock object.
...
hcr sets a catch or fishing mortality based on current stock status and reference points. 

Constraints can also be set (i.e. inter-annual bounds on TACs or effort), rules can have precendence, e.g. SSB must increase each year in a recovery plan is modelled.

fwdWindow sets up future dynamics of the FLStock object. 

fwdControl sets up the target options in the projections. It is very flexible but can be tricky to set up so there are a variety of methods for standard tasks, e.g. simulating a Harvest Control Rule (HCR) or running projections for F, catch and biomass target.
...
Recruitment is based on a stock recruitment relationship, this can be obtained by fitting to the historic time series.
